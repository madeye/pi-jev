...
retrieval, and `--jev-speed`/`--jev-skills` stay disabled.
...
The model identifier above matches this workspace's existing Pi configuration. Use your configured provider/model identifier for a different Qwen or DeepSeek server. The extension does not configure, download, or switch models.
...
The benchmark scripts launch Pi with **all proxy environment variables removed**, including uppercase/lowercase HTTP, HTTPS, ALL, NO_PROXY, and runtime/package-manager variants. This applies to hosted Jev calls made inside Pi as well as local-model calls. They do not change global shell or system proxy settings. When launching Pi manually, clear its proxy environment too; plugin hooks cannot retroactively reconfigure Pi's HTTP transport.
...
npm run bench:tps       # Throughput: context size, tool calls, TTFT and decode tok/s
...
Use `PI_BENCH_MODEL=provider/model npm run bench:local` for another configured model. Reports go under ignored `results/`; no secrets are written by the evaluation scripts. The local benchmark alternates pair order, includes process startup and Jev latency, records tokens and exact-match answers, and uses the same decision helper to add a skill hint. It isolates skill selection rather than invoking the full plugin hook. It is a five-case smoke benchmark, not proof of improved coding quality. Server settings may override Pi's requested `thinking=off`.
